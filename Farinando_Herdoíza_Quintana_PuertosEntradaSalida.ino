...
int pos0 = 2; // variable  de posción del primer switch--------control de luces
int pos1 = 3; // variable de posición del segundo switch------control de luces
int pos2 = 4; //variable de posción del tercer switch ------control de luces
int pos3 = 5; //variable de posción del cuarto switch ------control de luces
int pulsa = 6; //variable de posción del pulsador  ------control de contador
int  i = 0; //variable de recorrido del vector
int control = 0; //variable que permitirá controlar los swtichs
int a = 0;//variable de acarreo para el contador primer bit
int b = 0;//variable de acarreo para el contador segundo bit
int c = 0;//variable de acarreo para el contador tercer bit
int d = 0;//variable de acarreo para el contador cuarto bit
int e = 0;//variable de acarreo para el contador quinto bit
int f = 0;//variable de acarreo para el contador sexto bit
int btn = 7;
void setup() {
  for (; i < 6; i++) //configuración de pines de forma grupal
  { pinMode(leds[i], OUTPUT); //configuraciones los pines del vector como salidas
  }
  i = 0; //reseteo de la variable de conteo
  //declaración de las entradas de acuerdo a los pines
  pinMode(pos0, INPUT);
  pinMode(pos1, INPUT);
  pinMode(pos2, INPUT);
  pinMode(pos3, INPUT);
...
void loop() {
  if (digitalRead(pos0) == HIGH) //condición que cambiará la variable de control para el primer switch
    control = 1;
  if (digitalRead(pos1) == HIGH) //condición que cambiará la variable de control para el segundo switch
    control = 2;
  if (digitalRead(pos2) == HIGH) //condición que cambiará la variable de control para el tercer switch
    control = 3;
  if (digitalRead(pos3) == HIGH) //condición que cambiará la variable de control para el cuarto switch
    control = 4;
  if (digitalRead(pulsa) == HIGH) //condición que cambiará la variable de control para el pulsador
    control = 5;
  if (digitalRead(pos3) == LOW && digitalRead(pos2) == LOW && digitalRead && (pos1) == LOW && digitalRead(pos0) == LOW)
    //condición que controlará que cuando se apaguen todos los switchs el sistema se apague
    control = 5;
    //-------------------------------------------------------------------------------------------
  switch (control) //se utiliza un ciclo switch con una variable de control para cada juego de luces
  {
    case 1 ://juego de luces impares
      ...
      //condición que controla el estado de on of del pulsador ,si lee que se tiene estado bajo saldrá del ciclo sin realizar acción alguna
      else {
        for (; i < 6; i++)
        {
          if (i % 2 == 1) //condición que verificará el residuo de la dividir la posición entre 2,si dicho residuo es igual a uno pasa a encender los leds
          { digitalWrite(leds[i], HIGH); //encendido de leds según la condición
            delay(200);
          }
        }
        ...
        for (; i >= 0; i--) //
        {
          if (i % 2 == 1) //condición que verificará el residuo de la dividir la posición entre 2, si dicho residuo es igual a uno pasa a apagar  los leds
    ...
    case 3 ://juego de luces aleatorias
      ...
      //condicion que controla el estado de on of del pulsador ,si lee que se tiene estado bajo saldra del ciclo sin realizar accion alguna
        ...
        //si el pulsador esta activado se activa el respectivo código
        for (; i < 6; i++)
        { long  alea = random(6); //
    ...
    case 4 ://juego normal de luces
      ...
      //condicion que controla el estado de on of del pulsador ,si lee que se tiene estado bajo saldra del ciclo sin realizar accion alguna
        ...
        for (; i < 6; i++)
        {
          digitalWrite(leds[i], HIGH); //encendido de leds segun la condición
          delay(200);
        }
        //no se reinicia la variable de conteo ya que sera necesaria para apagar los leds
        for (; i >= 0; i--) //
    ...
    case 5 ://Contador Binario
      ...
      //condicion que controla el estado de on of del pulsador ,si lee que se tiene estado bajo saldra del ciclo sin realizar acción alguna
      else {
        //si el pulsador esta activado se activa el respectivo codigo
